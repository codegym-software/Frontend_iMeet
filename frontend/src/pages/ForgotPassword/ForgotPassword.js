...
export default function ForgotPassword() {
  ...
  const handleResetPassword = async (e) => {
    e.preventDefault();
    // validate against the passwordValidity checklist
    const allValid = Object.values(passwordValidity).every(Boolean);
    if (!allValid) {
      showMessage('Mật khẩu mới chưa đáp ứng đầy đủ yêu cầu', true);
      return;
    }
  ...
  return (
    <div className="forgot-password-container">
      <div className="forgot-password-box">
        ...
        {message && (
          <div className={`forgot-password-message ${messageIsError ? 'error' : 'success'}`}>
            {message}
          </div>
        )}
        ...
        {step === 3 && (
          <form onSubmit={handleResetPassword} className="forgot-password-form">
            ...
            {/* Password requirements box (moved between New Password and Confirm Password) */}
            <div className="password-requirements">
              <div className="requirements-header">Mật khẩu phải có:</div>
              <ul className="requirements-list">
                <li className={passwordValidity.length ? 'valid' : 'invalid'}>Ít nhất 8 ký tự</li>
                <li className={passwordValidity.uppercase ? 'valid' : 'invalid'}>Ít nhất 1 ký tự viết hoa</li>
                <li className={passwordValidity.lowercase ? 'valid' : 'invalid'}>Ít nhất 1 ký tự viết thường</li>
                <li className={passwordValidity.digit ? 'valid' : 'invalid'}>Ít nhất 1 số</li>
                <li className={passwordValidity.special ? 'valid' : 'invalid'}>Ít nhất 1 ký tự đặc biệt</li>
              </ul>
            </div>
