...
const DeviceList = () => {
  ...
  const [loading, setLoading] = useState(preloadedLoading || devicesLoading);
  ...
  useEffect(() => {
    setLoading(deviceTypesLoading || devicesLoading || preloadedLoading);
  }, [deviceTypesLoading, devicesLoading, preloadedLoading]);
  ...
  const handleAdd = async () => {
    ...
    if (Object.keys(errors).length === 0) {
      try {
        ...
        // Refresh list and update all contexts
        const list = await reloadDevices(deviceTypes);
        setDevices(list);
        setContextDevices(list);
        setPreloadedDevices(list);
