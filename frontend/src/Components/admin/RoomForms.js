        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '20px' }}>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                // Allow the user to type freely (including IME) and store the raw value.
                // We'll validate and coerce to an integer on submit to avoid interrupting typing.
            ...
            {devices.length === 0 ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '8px' }}>
                {devices.map((device, idx) => (
                  <label key={device?.id ?? `device-${idx}`} style={{ 
                    display: 'flex', 
                    alignItems: 'center', 
                    padding: '8px', 
                    backgroundColor: 'white',
                  ...
                  onMouseLeave={(e) => {
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: '12px', color: '#666' }}>
                        {(device?.deviceTypeName ?? '')} • SL: {(device?.quantity ?? 0)}
                      </div>
                    </div>
                  </label>
                ))}
