    ...
    <div style={{ 
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <div style={{ 
          ...
          {(() => {
            // Filter devices: show only devices with quantity > 0 OR already selected
            const availableDevices = devices.filter(device => {
              const isSelected = (formData.selectedDevices || []).includes(device?.id);
              const hasQuantity = (device?.quantity || 0) > 0;
              return hasQuantity || isSelected; // Show if has stock OR already selected
            });
            ...
            return (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '8px' }}>
                ...
                return (
                  <div key={device?.id ?? `device-${idx}`} style={{
                    padding: '8px',
                    ...
                    {isSelected && (
                      <div 
                        style={{ display: 'flex', alignItems: 'center', gap: '8px', paddingLeft: '28px' }}
                        onClick={(e) => e.stopPropagation()}
                      >
                        <label 
                          style={{ fontSize: '12px', color: '#495057', fontWeight: '500', minWidth: '60px' }}
                          onClick={(e) => e.stopPropagation()}
                        >
                          Số lượng:
                        </label>
                        ...
                        />
                        <span style={{ fontSize: '12px', color: '#666' }}>/ {maxQuantity}</span>
                      </div>
                    )}
                  </div>
                );
