...
const UserManagement = () => {
  ...
  const [currentPage, setCurrentPage] = useState(preloadedCurrentPage);
  const [totalPages, setTotalPages] = useState(preloadedTotalPages);
  const [totalElements, setTotalElements] = useState(preloadedTotalElements);
  ...
  useEffect(() => {
    ...
    setTotalPages(preloadedTotalPages);
    setTotalElements(preloadedTotalElements);
    setCurrentPage(preloadedCurrentPage);
  ...
  }, [preloadedUsers, preloadedLoading, preloadedStats, preloadedTotalPages, preloadedTotalElements, preloadedCurrentPage]);

  // Load users from API (for search and pagination)
  const loadUsers = async (page = 0, search = '') => {
    try {
      setLoading(true);
      const response = await reloadUsers(page, itemsPerPage, 'createdAt', 'desc', search);
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (searchTerm) {
        ...
        // Reset to preloaded data when search is cleared
        setUsers(preloadedUsers);
        setTotalPages(preloadedTotalPages);
        setTotalElements(preloadedTotalElements);
        setCurrentPage(preloadedCurrentPage);
  ...
  }, [searchTerm, preloadedUsers, preloadedTotalPages, preloadedTotalElements, preloadedCurrentPage]);
  ...
  const handleDelete = async (id) => {
    ...
    if (window.confirm(`Bạn có chắc chắn muốn xóa người dùng "${userToDelete?.fullName}"?`)) {
      try {
        ...
        await loadUsers(currentPage, searchTerm); // Reload current page
